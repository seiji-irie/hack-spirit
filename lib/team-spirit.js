...
let statusUrl   = 'https://teamspirit-8277--teamspirit.ap2.visual.force.com/apex/AtkWorkComponent';
let workTimeUrl = 'https://teamspirit-8277--teamspirit.ap2.visual.force.com/apex/AtkWorkTimeView';
...
class TeamSpirit {
  ...
  * report(type, date, note, isPersonalReason) {
    ...
    yield this.needChangeMonth(date).then((needChangeMonth) => {
      ...
      return this.nightmare
                 .click(`#empApplyDone${result.index}`)
                 .waitForProgressHidden();
    });
  }
  ...
  * setWorkTime(date, start, end) {
    let okButton = '#dlgInpTimeOk';
    let startBox = '#startTime';
    let endBox = '#endTime';
    ...
    yield this.showWorkTimeOfDayDialog(date).then(() => {
      ...
      return this.nightmare
        ...
        let message = 'specified date maybe a holiday.';
        return Promise.reject(new HackSpiritError(message));
      }
    ).then((result) => {
      if (!result.success) {
        let message = 'Cannot set work time. ' +
              'The work time conflicts. ' + result.message;
        return Promise.reject(new HackSpiritError(message));
      }
      return this.nightmare.waitForProgressHidden();
    });
